...
const getAllCourses = asyncWrapper(async (req, res) => {
  ...
  // Fetch all courses from the database using Course model
...
const addCourse = asyncWrapper(async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    const error = new appError.create(errors.array(), 400, httpStatusText.FAIL);
    return next(error);
  }
...
// we can use express-async-handler package also
